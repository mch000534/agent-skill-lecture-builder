...
[callout type="tip" title="工作紙下載"]
- [下載 A3 海報練習工作紙](assets/worksheets/poster-a3.svg)
[/callout]

### 起始媒介與預置材料
- 同一份 SVG 已包含完整散亂素材、三個縮圖框、A3 主版面框與字級、色彩、QR Code 位置註記欄，不另提供獨立數位模板
- 紙本組由教師在課前以 100% 比例列印為實際 A3（297 × 420 mm），不可使用「符合頁面」縮放；另備鉛筆、黑色筆與一支色筆
- 數位組由學員在課前開啟或匯入同一份 SVG，將它設為鎖定底圖，再於底圖上方新增文字與標註
- 工作紙尚未下載、列印或分發完成，或數位底圖尚未匯入並鎖定時，不開始 40 分鐘計時
- 紙本學員在工作紙的縮圖框與 A3 主框內抄寫必要短語；數位學員在鎖定底圖上新增必要文字與註記，不使用可移動的預置文字框
...
### 可用材料與畫框方式
- 本頁已提供完成練習所需的情境、原始文字與數據；學員另備兩張 A4 空白紙與筆即可開始，不需下載或取得額外模板
- **第一張 A4** 橫放，置中畫一個約 **24 × 13.5 公分**的 16:9 主投影片框；四周保留合理邊界，主框專門用於完成低保真改版
- **第二張 A4** 放置兩個各約 **8 × 4.5 公分**的 16:9 縮圖，並保留字級、圖表、刪減、資料限制與設計理由註記區；縮圖只用方框、文字線與大數字探索層級
...
- **第一張 A4，桌面版**：紙張橫放，置中畫一個 **14.4 × 18 公分**的長頁面框，代表 1440 × 1800；從框頂向下 **9 公分**畫 fold 線，代表 1440 × 900 的第一個視窗，紙面上下仍保有約 1.5 公分邊界
- **第二張 A4，手機版**：紙張直放，置中畫一個 **6 × 26 公分**的長頁面框，代表 390 × 1688；從框頂向下 **13 公分**畫 fold 線，代表 390 × 844 的第一個視窗，紙面上下仍保有約 1.85 公分邊界
- 在 fold 線下方延伸頁面，依序安排價值、流程、FAQ 與頁尾 CTA；框旁用箭頭標示桌面多欄如何收合為手機單欄
- 桌面框與手機框使用不同紙上比例尺，只檢查各自的資訊層級、區塊順序與 fold 前內容，不比較兩張紙上的物理尺寸
...
# 第三節：從元件到系統
> 本節 120 分鐘，從 UI 元件與狀態開始，建立可重複的設計規則，完成場地預約 App 低保真流程，再用回饋與迭代方法整合城市創意節的海報、PPT、Web 與 App 草案。
...
### 元件不只是外觀，也是行為承諾
- **按鈕**：文字要說明會發生的行動，例如「確認預約」比「送出」清楚；主要、次要與危險行動需有穩定層級
- **表單**：標籤、輸入格式、必填提示、說明、錯誤與送出結果要形成完整對話，不可只留下空白框
- **卡片**：把同一任務需要一起比較的標題、摘要、狀態與行動組成單位；整張卡可點擊時，仍需讓鍵盤使用者知道焦點位置
- **導航**：名稱使用使用者熟悉的詞，清楚標示目前位置、可前往位置與返回方式；不可只靠圖示猜測
- 元件的名稱、位置與行為一旦重複，就形成使用者預期；同一行動不應在不同畫面忽然改名或改變位置

### 互動元件的常見狀態，依元件與平台選用
- 狀態不是每個元件都要套用的固定清單；先確認元件能否點擊、輸入、選取或表示目前位置，再選擇需要呈現的狀態
- **預設**：尚未互動時，標籤、可點擊範圍與主要行動清楚
- **Hover**：滑鼠移入時提供可互動線索，但觸控裝置沒有 Hover，因此不能把必要資訊只藏在此狀態
- **Pressed / Active**：按下當下提供操作回饋，或標示目前頁面與正在使用的功能；兩種語意需在規則中清楚命名
- **Focus**：鍵盤 Tab 移入時顯示清楚焦點框；焦點順序應跟畫面閱讀順序一致
- **Selected**：卡片、選項或篩選條件被選取後，需以文字、勾選、邊框或形狀說明目前選擇
- **Disabled**：不可操作時仍可辨認，並在需要時解釋原因；不能只把按鈕變成難以閱讀的淺灰
- **載入**：送出或取得資料時防止重複操作，保留進度訊息，例如「正在確認名額」
- **成功**：明確說明已完成什麼、保留確認資訊，並提供下一步
- **錯誤**：指出發生位置、原因與修正方式，保留已正確輸入的內容並提供重試

### 不同元件需要不同狀態組合
- **按鈕**：通常需要預設、Hover、Pressed、Focus、Disabled 與載入；觸控介面不依賴 Hover
- **輸入欄**：重點是預設、Focus、已填寫、Disabled、錯誤與必要的成功確認，不需要 Selected
- **可選卡片**：需要預設、Hover、Focus、Selected 與 Disabled；純資訊卡片若不可操作，就不應假裝有 Hover 或 Pressed
- **導航**：需要預設、Hover、Focus 與 Active，清楚標示目前頁面；若含可勾選篩選器，再加入 Selected
- 成功與錯誤常屬於欄位、表單或整個流程的回饋，不必強迫每一顆按鈕各自擁有相同狀態

[flow]
1. 看得見 — 元件的角色、邊界、標籤與目前狀態可辨認
2. 到得了 — 滑鼠、觸控與鍵盤都能抵達主要操作
3. 做得懂 — 行動名稱能預期結果，輸入格式在出錯前已說明
4. 有回應 — 依元件提供 Hover、Pressed、Focus、Selected、載入、成功或錯誤等必要回饋
5. 能恢復 — 失敗時保留資料、指出修正方式並允許返回或重試
[/flow]

### 一致性與可及性是基本功能
- 相同目的使用相同元件、名稱、位置與狀態規則；差異只在任務真的不同時出現
- 文字與背景需有清楚對比，可點擊範圍需足夠，表單標籤不可在輸入後消失
- 鍵盤使用者必須看得見 Focus，不可用 `outline: none` 移除焦點而沒有替代樣式
- 錯誤訊息放在相關欄位附近，寫出「電子郵件需包含 @」等修正方式，不只顯示「格式錯誤」
- 成功、警告、錯誤與已額滿不可只靠綠、黃、紅區分；需同時使用文字、圖形、邊框或狀態標籤
- 動畫與載入效果應幫助理解狀態，不應讓使用者猜測系統是否仍在運作

[compare label-left="難以操作的具體例子" label-right="可理解且可恢復的改法"]
- 藍色按鈕只寫「送出」，送出後畫面沒有變化 | 按鈕寫「確認預約」，載入時顯示「正在確認名額」並防止重複點擊
- 表單只用框內提示「Email」，輸入後標籤消失 | 框外保留「電子郵件」標籤，另寫格式例子與用途
- 錯誤欄只變紅，不說明問題 | 欄位加上錯誤標籤與「請輸入可收信的完整地址」文字
- Disabled 按鈕變成幾乎看不見的淺灰 | 保持文字可讀，並說明「請先同意場地規則」
- 鍵盤 Tab 到按鈕時沒有任何變化 | 顯示高對比 Focus 外框，順序由欄位到主要行動再到返回
- 成功後直接跳回首頁 | 顯示預約編號、日期、場地、下一步與修改入口
[/compare]

[callout type="info" title="20 分鐘教學節奏"]
- 0–4 分鐘：辨認按鈕、表單、卡片與導航各自支持的任務
- 4–9 分鐘：比較按鈕、輸入欄、可選卡片與導航需要的不同狀態組合
- 9–14 分鐘：比較六組具體好壞例，找出命名、回饋與恢復問題
- 14–18 分鐘：用鍵盤 Focus、欄位錯誤與不只靠顏色三項要求走查預約表單
- 18–20 分鐘：每人挑一個元件，口頭說出遺漏狀態與一項修正
[/callout]

> **介面不是一張靜止畫面**
> 使用者會移入、聚焦、輸入、等待、成功與失敗。只有預設畫面好看，仍不足以支持完整任務。
...
### 設計系統保存的是決策，不是元件收藏
- 設計系統把反覆出現的色彩、字級、間距、元件與狀態寫成可重用規則，讓團隊面對新畫面時有共同判斷依據
- 只把二十個按鈕截圖排在一起，卻沒有命名、用途、狀態與限制，是元件收藏，不是可運作的系統
- 規則需要說明「何時使用、何時不用、如何組合、出錯時怎麼呈現」，並隨測試結果持續修正
- 一致性不是所有媒介長得完全一樣，而是相同角色、層級與行動維持可辨認的關係

### 四組起始規則
- **色彩 Token**：本練習四個起始 Token 固定為 `color-text-primary`、`color-surface`、`color-action`、`color-error`，不要只寫「深藍」或「紅色」
- **字級階梯**：先定義顯示標題、章節標題、正文、註解的尺寸、字重與行距；角色相同就沿用，不逐頁憑感覺調整
- **8pt 間距**：以 8 為基礎建立 8、16、24、32、48 等間距，較小的 4 可處理緊密細節；相同關係使用相同距離
- **元件規則**：記錄按鈕、輸入欄、卡片與導航的尺寸、內距、標籤、狀態、Focus、錯誤與使用限制

[callout type="tip" title="15 分鐘辨識活動"]
- 0–4 分鐘：由圖例辨認 Token、字級階梯、8pt 間距與元件狀態
- 4–8 分鐘：比較「六個漂亮按鈕截圖」與「有名稱、用途、狀態、限制的按鈕規則」
- 8–12 分鐘：兩人圈出一個現有畫面中可改寫成 Token 或重用規則的三項決策
- 12–15 分鐘：互換規則，檢查另一人是否能不靠猜測重建相同層級與狀態
[/callout]

[compare label-left="元件收藏" label-right="可使用的設計系統"]
- 色票只標示十六進位色碼 | 色彩以 `color-text-primary`、`color-surface`、`color-action`、`color-error` 四個起始 Token 命名
- 按鈕只展示不同顏色 | 定義主要、次要、危險用途及所有互動狀態
- 每頁看起來相似，但尺寸靠目測 | 字級與 8pt 間距形成可重複的數值規則
- 規則只有設計師看得懂 | 名稱、例子、使用限制與可及性要求都可被查證
[/compare]
...
[callout type="info" title="圖例可直接對照本次練習"]
圖例與下方任務共用 2026 年 10 月 17、18、24 日、09:00–11:00 等時段，以及「選擇時段」「填寫資料」「檢查預約」「保留名額」等具體按鈕。右側亦呈現名額已保留但仍待付款，以及時段剛額滿後返回重選的狀態，可直接作為四框走讀參考。
[/callout]

### 紙框、工具與交付方式
- 使用一張 A4 紙橫放，畫四個各約 **48 × 104 mm** 的手機框，框與框之間留約 8 mm；四框總寬約 216 mm，可在 297 mm 紙寬內居中並保留兩側註記空間
- 四個紙框依序代表選日期、選時段、填資料、確認；框內只畫方塊、標籤、短文字、按鈕與狀態，不畫裝飾圖
- 另以框外箭頭或小註記表示載入、已額滿、欄位錯誤、提交錯誤與成功，不另畫完整精修頁
- 紙本只需 A4、鉛筆與深色筆；若使用熟悉的設計工具，可建立四個 390 × 844 畫框，但不搜尋 UI Kit、圖示或圖片
- 交付成果是可走讀的四步低保真流程，必須看得出目前步驟、已選內容、返回修改、主要行動與錯誤恢復方式

### 任務情境：場地預約
- 主要使用者：學生專題小組召集人，以手機替 6 人團隊預約討論與原型製作空間
- 場地：城市創意中心，多功能室 A；容納 2–12 人，提供大桌、白板、投影與無障礙入口
- 費用：每兩小時 300 元；送出後先保留 10 分鐘，完成付款才正式成立
- 開放日期：2026 年 10 月 17 日星期六、10 月 18 日星期日、10 月 24 日星期六
- 時段：09:00–11:00 尚有 2 組、11:30–13:30 已額滿、14:00–16:00 尚有 1 組、16:30–18:30 尚有 3 組
- 必填資料：預約人姓名、電子郵件、手機、使用人數、活動目的、是否需要無障礙支援、同意場地規則
- 場地規則：不可使用明火；飲食限有蓋飲品；離場前復原桌椅；最晚於使用前 48 小時取消
- 成功條件：使用者能選定日期與可用時段、填完資料、確認摘要並知道預約是否成立及下一步

### 四步畫面必備內容

[flow]
1. 選日期 — 顯示月份、可選日期、已選日期、返回與「下一步：選時段」；取得可用日曆時呈現載入狀態
2. 選時段 — 同時顯示時間、費用與剩餘組數；已額滿時保留文字狀態但不可選，並允許改日期
3. 填資料 — 保留欄位標籤、格式例子、必填與場地規則；欄位錯誤靠近欄位顯示並保留其他正確資料
4. 確認 — 列出場地、日期、時段、人數、費用、聯絡方式與取消規則；可返回修改，送出時顯示載入，成功後顯示編號與付款期限
[/flow]

### 必須補上的狀態
- **預設**：主要按鈕名稱對應下一步，步驟指示清楚，目前選擇可辨認
- **載入**：日期或時段資料載入時顯示正在取得；確認送出時顯示正在保留名額並防止重複點擊
- **已額滿**：11:30–13:30 顯示「已額滿」，不可只用灰色或紅色，也不可讓人誤以為仍能點選
- **欄位錯誤**：電子郵件缺少 @ 時顯示「請輸入完整電子郵件，例如 name@example.com」，焦點移到第一個錯誤欄位
- **提交錯誤**：名額在送出前被訂走時，保留資料並顯示「此時段剛額滿，請重新選擇時段」與返回按鈕
- **成功**：顯示預約編號 `CC-A-1024-1430`、摘要、「名額保留至 14:40」與「前往付款」
- 狀態均需使用文字與結構提示，不用顏色作為唯一線索

### 25 分鐘分配

[timeline]
- 0–4 分鐘 | 觀察問題 | 圈出四步任務、必要資料、已額滿與欄位錯誤等關鍵分支
- 4–7 分鐘 | 套用原則 | 指定共同導航、主要按鈕、已選狀態、Focus、錯誤與返回方式
- 7–17 分鐘 | 動手改版 | 在一張 A4 橫式紙畫四個 48 × 104 mm 紙框，完成日期、時段、資料與確認低保真畫面
- 17–21 分鐘 | 補上狀態 | 以框外註記加入載入、成功、提交錯誤、已額滿與欄位錯誤
- 21–25 分鐘 | 對照檢查 | 由同伴從第一步走讀到成功，再抽一個錯誤狀態測試恢復路徑並修正
[/timeline]

### 從觀察到檢查

[flow]
1. 觀察 — 找出每一步必須做的判斷、要保留的資料與可能中斷任務的狀態
2. 原則 — 固定步驟位置、按鈕名稱、選取樣式、Focus、錯誤與返回規則
3. 改版 — 用四個紙框完成主要路徑，再在框外補上載入、已額滿、錯誤與成功狀態
4. 檢查 — 讓同伴不受提示走讀正常與錯誤路徑，記錄卡住的位置並修正
[/flow]

[callout type="warning" title="實作限制"]
- 只能使用一張 A4 橫式紙與四個主要畫框，不另做視覺精修頁
- 四步固定為選日期、選時段、填資料、確認，不新增登入、會員中心或優惠流程
- 每一步只保留一個主要行動，返回修改必須可見但權重較低
- 已額滿與錯誤不得只靠顏色；Disabled 文字仍需可讀，鍵盤 Focus 以文字註記表示
- 送出錯誤必須保留已輸入資料並提供恢復方式，成功畫面必須說明預約尚待付款
- 不搜尋照片、插圖、圖示、字體或 UI Kit，只用方框、文字層級、箭頭與狀態註記
[/callout]

### 對照檢查清單
- [x] 四步順序完整，使用者隨時知道目前位置與下一步
- [x] 日期、時段、費用與剩餘名額在選擇前可比較
- [x] 已額滿時段有文字標籤且不可選，不只依賴灰色或紅色
- [x] 欄位標籤、格式、必填與錯誤修正方法都清楚
- [x] 錯誤發生後保留正確資料，能返回改日期或時段並重試
- [x] 確認頁可核對場地、日期、時段、人數、費用與取消規則
- [x] 載入時防止重複操作，成功時顯示編號、保留期限與付款下一步
- [x] 主要按鈕、返回方式、Focus 與狀態規則跨四頁一致
- [x] 同伴能走完正常路徑，也能從一個錯誤狀態恢復

## 回饋與迭代（15 分鐘）

### 回饋要連回目標與證據
- **目標**：先重述使用者、情境與任務，確認評圖不是在比較個人風格偏好
- **證據**：描述實際看到、找到、誤解或無法完成的行為，例如「三秒後兩位測試者都沒有說出日期」
- **優先級**：先修阻止主要任務、造成誤解或無法恢復的問題，再處理次要一致性與美化
- **再測試**：每輪選一至兩項可觀察改動，用相同任務與條件再次測試，確認問題是否改善

[compare label-left="只有偏好的回饋" label-right="可以採取行動的回饋"]
- 我不喜歡這個顏色 | 主要 CTA 與次要標籤都用同一藍色，三位測試者有兩位先點錯；建議降低次要標籤權重再測
- 這頁很亂 | 日期、費用與限制分散在三處，測試者來回搜尋；建議先依選擇任務重新分組
- 按鈕不夠漂亮 | 按鈕只寫「下一步」，使用者無法預期結果；建議改成「查看時段」並再次走讀
- 我會把這裡放大 | 五秒測試未讀到適合年齡；建議先提高年齡資訊的層級，再比較測試結果
- 這個流程怪怪的 | 送出錯誤後清空資料，使用者無法恢復；優先保留資料並提供返回選時段
[/compare]

### 評圖句型

```prompt [label="以目標與證據提出回饋"]
對於【主要使用者】在【情境】要完成【目標】，
我觀察到【具體行為或測試證據】，可能因為【版面、文字、流程或狀態原因】；
建議先【一項可執行改動】，再用【相同任務與條件】確認是否改善。
```

### 15 分鐘回饋流程

[timeline]
- 0–3 分鐘 | 重述目標 | 設計者用一句話說明使用者、情境、主要任務與限制
- 3–6 分鐘 | 描述證據 | 評圖者只說看到的順序、找不到的位置或走讀卡點，不先提出風格偏好
- 6–9 分鐘 | 改寫回饋 | 把一句「我不喜歡」改成包含目標、證據與可執行改動的句子
- 9–12 分鐘 | 排定優先級 | 先選一個阻礙主要任務的問題，再選一個一致性或可及性問題
- 12–15 分鐘 | 修正與再測試 | 立即改一處，寫下相同測試任務、成功條件與下一輪要觀察的證據
[/timeline]

[callout type="tip" title="優先順序判斷"]
- 第一優先：主要任務無法完成、資訊被誤解、錯誤後不能恢復
- 第二優先：重要資訊難找、跨頁規則不一致、鍵盤或對比造成障礙
- 第三優先：不影響理解與操作的視覺細節、裝飾與個人風格偏好
[/callout]

> **迭代不是無限修改**
> 每一輪都要有明確假設、改動與成功條件；若沒有新證據，只是不斷換風格，並不等於設計正在進步。
...
- **活動名**：城市創意節 2026
- **主題句**：把城市問題變成共同作品
- **活動內容**：兩日展覽、短講、步行觀察與共創工作坊，讓市民、學生與設計團隊一起提出可測試的城市改善方案
- **日期與時間**：2026 年 11 月 14–15 日，10:00–18:00
- **地點**：舊港創意倉庫 A、B 館；港灣站步行 6 分鐘
- **主要對象**：對城市生活、公共空間與創意實作有興趣的 18–35 歲市民與大專學生
- **活動亮點**：12 組城市提案展、8 場十五分鐘短講、3 條街區觀察路線、4 場九十分鐘共創工作坊
- **主舞台**：11 月 14 日 11:00「從抱怨到問題定義」、15:00「讓公共空間更容易使用」；11 月 15 日 11:30「小型測試如何改變城市」、16:00 成果分享
- **工作坊**：友善街角、夜間步行、公共座椅、社區指標；每場 24 人，須預約
- **票價**：展覽與短講免費；街區觀察 80 元；共創工作坊 180 元
- **報名規則**：每人可選一場街區觀察及一場工作坊；付款前保留名額 10 分鐘；額滿可登記候補
- **主要 CTA**：查看節目並報名
- **報名短網址**：citycreate.example.org/2026
- **信任與支援資訊**：提供粵語與華語場次、無障礙入口與座位、服務台可借用聽力輔具
- **主辦單位**：城市共創研究室；協辦單位：舊港文化區、青年設計聯盟
- **聯絡方式**：hello@citycreate.example.org，+853 6000 2026
- **取消方式**：活動前 7 日可退費；3–6 日可改期一次；2 日內不退費

### 先定義使用者與溝通目標
- 從完整素材中選定一位主要使用者，例如「第一次參加、用手機搜尋週末活動的大學生」，不可同時服務所有人
- 用一句話寫出情境、阻礙、任務與成功條件，例如「讓第一次進站的大學生在五秒內理解活動價值，找到適合的免費內容並開始報名」
- 海報、PPT、Web 與 App 必須支持同一目標，但可依觀看距離、時間與操作方式調整資訊量
- 每一個媒介只設定一個主要 CTA；若該媒介不直接操作，也要讓下一步明確

### 建立一組迷你規則
- **色彩**：只定義 `color-text-primary`、`color-surface`、`color-action`、`color-error` 四個 Token，寫用途即可，不需現場挑精確色碼
- **字體**：定義顯示標題、章節標題、正文與註解四層角色，最多兩套字體
- **間距**：以 8pt 為基礎，標註 8、16、24、32 四級關係
- **元件**：定義一個主要按鈕、一個資訊卡與一個狀態標籤，至少註明預設、Focus、Disabled 與錯誤規則
- 規則應在四個媒介保持角色一致，不要求尺寸完全相同；海報標題與 App 標題可以不同尺寸，但層級關係需一致

### 計時前先備妥兩張預印工作紙

[callout type="tip" title="工作紙下載"]
- [下載整合練習第 1 張工作紙](assets/worksheets/capstone-sheet-1.svg)
- [下載整合練習第 2 張工作紙](assets/worksheets/capstone-sheet-2.svg)
[/callout]

- 教師須在練習前完成下載，並以 100% 比例列印為兩張 A4 橫式工作紙；若採數位作答，也須先開啟兩份模板並確認可標註
- 材料尚未下載、列印或分發完成時，不開始 45 分鐘計時；學員拿到兩張預印工作紙與筆後才開始

### 兩張 A4 的固定交付
- **第 1 張預印工作紙**：直接在左側直式海報框與右側 16:9 PPT 主視覺框內完成草案；其餘預留區寫共同 Token 與決策理由
- **第 2 張預印工作紙**：直接在左側 Web 首頁長框與右側 2 × 2 App 報名流程框內完成草案；框旁標註 CTA、狀態與返回方式
- 海報只安排活動名、日期地點、亮點與 CTA；PPT 只用一個訊息式標題、一項主數字或主觀點與一項證據
- Web 只畫 Hero、三項價值、節目預覽、支援資訊、FAQ 入口與 CTA；App 只畫選活動、選場次、確認資料、成功或候補
- 全部成果只用方塊、文字層級、閱讀箭頭、CTA 與 Token 註記，不做精修、圖片搜尋、完整文案、Logo、插圖或可點擊原型

[callout type="warning" title="整合任務限制"]
- 只能使用兩張已提供的預印 A4 工作紙，不能為每個媒介另開一張紙，也不在計時內從空白紙量框
- 必須使用同一主要使用者、溝通目標與迷你設計規則，不為四個媒介各自發明風格
- 四個草案都要有清楚第一焦點與下一步，但不需放入完整活動素材
- 海報、PPT、Web 與 App 都是低保真；只畫支持層級、動線與狀態的必要內容
- 不搜尋圖片、模板、字體或參考網站，不製作可掃描 QR Code，不調整像素級細節
- 若時間不足，先保留目標、層級、CTA、流程與 Token 註記，不增加裝飾
[/callout]

### 45 分鐘精確分配

[timeline]
- 0–5 分鐘 | 定義目標 | 選定主要使用者，寫下溝通目標、使用情境與成功條件
- 5–12 分鐘 | 建立規則 | 定義色彩、字體、8pt 間距、按鈕、卡片與狀態標籤規則
- 12–22 分鐘 | 完成海報 | 在第 1 張 A4 左區安排主題、日期地點、亮點與 CTA
- 22–29 分鐘 | 完成 PPT | 在第 1 張 A4 右區安排訊息式標題、單一主觀點與證據
- 29–36 分鐘 | 完成 Web | 在第 2 張 A4 左區安排 Hero、價值、節目預覽、支援資訊與 CTA
- 36–41 分鐘 | 完成 App | 在第 2 張 A4 右區畫選活動、選場次、確認資料與成功或候補
- 41–45 分鐘 | 五項互評 | 快速掃描目標、層級、順暢、一致與理由，只輸出一項具體證據與一項優先修正，不在計時內改稿
[/timeline]

### 五項互評標準
- **目標**：是否能說出主要使用者、情境與每個媒介要支持的同一任務
- **層級**：是否每個草案都有唯一第一焦點，重要資訊與 CTA 可快速辨認
- **順暢**：閱讀與操作是否由理解、判斷走向行動；App 出錯後是否能返回或恢復
- **一致**：色彩、字體、間距與元件角色是否跨媒介維持同一規則
- **理由**：是否能用目標、限制或測試證據說明至少三項取捨，而不是只說個人喜好

[flow]
1. 快速掃描 — 逐張查看第一眼焦點、預期閱讀順序、主要 CTA 與可能卡住的位置
2. 五項對照 — 用目標、層級、順暢、一致與理由檢查四個媒介，不逐項展開長篇討論
3. 圈選優先 — 只圈出一個最影響理解、行動或跨媒介一致性的問題
4. 輸出回饋 — 寫下一項具體證據與一項優先修正，改稿與再測試留待下一輪
[/flow]

[quiz type="single"]
Q: 哪一項最能說明「設計系統」而不只是元件收藏？
- [ ] 把所有做過的按鈕截圖集中到一頁
- [ ] 讓海報、PPT、Web 與 App 使用完全相同的字體尺寸
- [x] 定義色彩、字級、間距與元件的名稱、用途、狀態和限制，並用測試結果持續更新
- [ ] 每次設計新畫面時先挑一組不同風格，增加變化
Hint: 系統的價值在於保存可重用的決策與使用條件。
[/quiz]

[summary]
- **設計思維** | 從使用者、情境、問題、目標與限制開始，讓每個視覺選擇都有可驗證理由
- **視覺基礎** | 用色彩、字體、留白、對比、對齊與層級建立清楚閱讀順序
- **構圖與版面** | 以網格、親密性、重複與閱讀動線安排注意力，讓內容由焦點走向行動
- **UX 與資訊架構** | 依使用者任務分組與命名資訊，建立可找到、可完成、可預防錯誤也可恢復的流程
- **UI 與設計系統** | 為按鈕、表單、卡片、導航與狀態定義一致、可及且可重用的 Token 與元件規則
- **回饋與迭代** | 以目標、行為證據、優先級與再測試取代個人偏好，讓每輪修改都能被驗證
- **跨媒介整合** | 在海報、PPT、Web 與 App 中維持同一溝通目標與設計角色，並依媒介情境調整資訊量與互動方式
[/summary]
