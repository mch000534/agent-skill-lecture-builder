...
4. 有回應 — Hover、Focus、載入、成功與錯誤都提供及時回饋
...
- 4–9 分鐘：逐一走過預設、Hover、Focus、Disabled、載入、成功與錯誤狀態
...
### 四組起始規則
- **色彩 Token**：用功能命名，例如 `color-text-primary`、`color-surface`、`color-action`、`color-error`，不要只寫「深藍」或「紅色」
...
- 色票只標示十六進位色碼 | 色彩以文字、表面、行動、成功與錯誤等功能命名
...
### 建立一組迷你規則
- **色彩**：只定義文字、表面、主要行動與錯誤四個 Token，寫用途即可，不需現場挑精確色碼
...
### 兩張 A4 的固定交付
- **第 1 張 A4 橫放**：左區畫約 75 × 105 mm 的直式海報框，右區畫約 128 × 72 mm 的 16:9 PPT 主視覺框；其餘空間寫共同 Token 與決策理由
- **第 2 張 A4 橫放**：左區畫約 80 × 155 mm 的 Web 首頁長框，右區以 2 × 2 排列四個約 24 × 52 mm 的 App 報名關鍵流程框；框旁標註 CTA、狀態與返回方式
...
- 41–45 分鐘 | 五項互評 | 依目標、層級、順暢、一致與理由各說一句證據，選一項優先修正
[/timeline]
...
[flow]
1. 觀察 — 逐張指出第一眼焦點、預期閱讀順序、主要 CTA 與可能卡住的位置
2. 對照 — 用同一主要使用者、溝通目標與迷你規則檢查四個媒介
3. 排序 — 先處理阻礙理解或行動的問題，再處理一致性與視覺細節
4. 修正 — 在兩張 A4 上直接改一處，寫下預期改善與下一次測試方式
[/flow]
