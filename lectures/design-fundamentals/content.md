...
- **PPT 例子**：訊息式標題「試辦使完成率提升 22 個百分點」→ 前後比較圖 → 一句解釋 → 資料來源；觀眾不需先讀完整段落才知道結論
...
### 起始媒介與預置材料
- 教師提供已放入全部原始素材的 **16:9 單頁簡報模板**，以及內容相同的 A4 橫式紙本工作紙；學員二選一，不從空白檔案開始
- 數位模板包含標題區、十二欄參考線、圖表區、證據註記區、資料來源區與三個縮圖框，文字與數據皆已放在可移動文字框中
- 紙本工作紙包含一頁密集原稿、可剪排的內容條、三個縮圖框、16:9 版面框，以及字級、圖表與刪減註記欄
...
- 單頁目標：十秒內理解試辦是否改善完成率，以及哪一項改變最值得擴大
- 核心結論：導入手機表單、即時庫存與到期提醒後，借用流程完成率由 61% 提升至 83%，增加 22 個百分點
- 成功條件：測試者看十秒後，能說出「完成率提升 22 個百分點」並指出一項支持證據
...
舊流程完成率：61%（2025 年同期 238 筆申請）
新流程完成率：83%（增加 22 個百分點）
平均填表時間：8 分 40 秒降至 3 分 10 秒，縮短 5 分 30 秒
櫃台補件率：34% 降至 12%，減少 22 個百分點
逾期比例：18% 降至 9%，減少 9 個百分點
滿意度：3.1 分提升至 4.3 分（滿分 5 分，回收 201 份）
三項改動：手機表單、即時庫存、到期前 24 小時提醒
質性回饋：多數學生認為可先確認庫存最有幫助；行政同仁認為資料完整度提高
限制：僅試辦 3 個學系、期間 8 週，跨學期成效仍需追蹤
建議：下一學期擴大至 6 個學系，保留提醒功能並測試英文介面
資料來源：器材中心系統紀錄、表單分析與試辦後問卷
```

### 一頁一重點的取捨
- 把「專案成果報告」改為能直接傳達判斷的訊息式標題，例如「新流程讓借用完成率提升 22 個百分點」
- 主圖只回答一個問題：完成率如何改變；使用 61% 與 83% 的前後比較條形圖或大數字，不把所有指標塞進同一張圖
- 次要證據最多保留兩項，例如填表時間縮短 5 分 30 秒、補件率減少 22 個百分點
- 改動項目以一句註解連結結果；限制與資料來源縮成可讀的小區塊，但不可刪除
- 建議移到口頭說明或下一頁，避免同一頁同時回答成果、方法、限制與未來計畫四個問題

[compare label-left="密集原稿" label-right="單頁結論版"]
- 標題只寫專案名稱，觀眾不知道結論 | 標題直接寫出完成率提升 22 個百分點
- 九項數據以相同字級排列 | 一項主指標配兩項次要證據
- 用長段文字交代三項改動 | 用短註記指出改動與結果的關係
...
- 5–10 分鐘 | 套用原則 | 寫出三個訊息式標題候選，選定一項主指標與最多兩項次要證據
- 10–17 分鐘 | 探索版面 | 在預置模板畫兩個縮圖，指定第一焦點、圖文比例與閱讀順序
- 17–27 分鐘 | 動手改版 | 完成一張黑白低保真投影片，標註標題、正文、數字與來源字級，以及圖表和強調色用途
- 27–32 分鐘 | 十秒測試 | 與同伴交換，展示十秒後詢問核心結論與支持證據
...
3. 動手改版 — 用預置 16:9 模板或工作紙完成低保真版面，補上字級、圖表與色彩註記
4. 對照檢查 — 用十秒口述結果對照核心結論，確認閱讀順序能從標題到證據再到限制
[/flow]

[callout type="warning" title="實作限制"]
- 只製作一張 16:9 投影片，不增加頁面，也不改動原始數值
- 第一層只能有一個訊息式標題與一項主指標；次要證據最多兩項
- 圖表只使用前後比較條形圖或大數字比較，不使用 3D、圓餅圖或裝飾性圖示
- 圖表與數字區建議占版面 45%–60%，長段文字不得直接貼回投影片
- 必須保留試辦對象或樣本、限制與資料來源，但可縮寫成短句
- 以低保真版面及註記為交付，不花時間搜尋照片、插圖、字體或動畫
[/callout]

### 對照檢查清單
- [x] 標題是一句可判斷的成果，而不是「成果報告」或章節名稱
- [x] 十秒內能辨認 61%、83% 與增加 22 個百分點的關係
- [x] 一項主指標取得最大權重，次要證據不超過兩項
- [x] 圖表選擇能準確比較前後數值，沒有誇大差異
...
### 起始媒介與預置模板
- 教師提供一份已放入完整素材的低保真模板：桌面版 1440 像素寬畫布與手機版 390 像素寬畫布並列，另有可移動的內容卡與元件框
- 模板預置 Hero、價值、流程、FAQ、CTA 五個區塊，以及標誌、導覽、標題、正文、按鈕、場次卡與頁尾元件
- 紙本替代版包含相同比例的桌面與手機線框、可剪排內容條、箭頭與註記區；學員可完全使用紙筆
...
- 單一行動路徑：理解活動 → 確認適合 → 查看流程與疑慮 → 點選「查看場次並報名」
...
- 8–13 分鐘 | 安排桌面版 | 在預置桌面模板完成低保真區塊配置，畫出從 Hero 到頁尾 CTA 的閱讀動線
- 13–19 分鐘 | 動手改版 | 補上關鍵內容、場次入口、按鈕文字與 FAQ 展開方式，不做精修
- 19–24 分鐘 | 轉為手機版 | 將雙欄或多欄內容收合成單欄，保留合理順序與可觸及的 CTA
- 24–27 分鐘 | 五秒走查 | 與同伴交換，確認適合年齡、活動價值、報名入口與下一步是否找得到
...
3. 動手改版 — 使用預置模板完成桌面與手機低保真線框，標出收合順序與行動連結
4. 對照檢查 — 以五秒口述與不提示點選測試，確認使用者能理解、找到並開始報名
[/flow]

[callout type="warning" title="實作限制"]
- 必須使用提供的五個內容區塊，不新增購物、會員、社群牆或裝飾性區塊
- 全頁主要 CTA 只使用「查看場次並報名」，Hero 與頁尾可重複，但不得出現另一個競爭行動
- Hero 必須在不捲動的首屏回答適合年齡、活動價值與主要 CTA；不得只放氛圍圖片或口號
